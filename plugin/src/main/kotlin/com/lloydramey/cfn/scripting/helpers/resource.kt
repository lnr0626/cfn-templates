...
class ResourceDelegate<out T : ResourceProperties>(val properties: T, val attributes: List<ResourceDefinitionAttribute>) : ReadOnlyProperty<CfnTemplateScript, Resource<T>> {
    override fun getValue(thisRef: CfnTemplateScript, property: KProperty<*>): Resource<T> = Resource(property.name, attributes, properties)
}
